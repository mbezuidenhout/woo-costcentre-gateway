...
class WC_Gateway_Costcentre extends WC_Payment_Gateway {
	...
	public function email_instructions( $order, $sent_to_admin, $plain_text = false ) {
		if ( ! $sent_to_admin && $this->id === $order->get_payment_method() && $order->has_status( 'on-hold' ) ) {
			//$this->requisition_form( $order->get_id() );
		}
	}
	...
	public function process_payment( $order_id ) {
		...
		do_action( 'woocommerce_' . $this->id . 'process_payment', $order_id );
	...
	public function init_payment_fields() {
		...
		apply_filters( 'woo_costcentre_gateway_form_fields', $this->gateway_fields );
	}
	...
	public function validate_fields() {
		...
		apply_filters( 'woo_costcentre_gateway_fields', $validate_fields );
